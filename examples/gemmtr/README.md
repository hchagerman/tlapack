...
The code uses the routine [tlapack::gemmtr](../../include/lapack/gemmtr.hpp) to perform the complete factorization in place, and [tlapack::ung2r](../../include/lapack/ung2r.hpp) to generate the m-by-n matrix Q. We store R in a n-by-n upper triangular matrix.
...
2. Using `make` on the same directory of [example_gemmtr.cpp](example_gemmtr.cpp). In this case, you may need to edit `make.inc` to set the environment variables needed by [Makefile](Makefile). After a successful build, the executable will be in the current directory.
