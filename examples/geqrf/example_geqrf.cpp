...
void run(size_t m, size_t n)
{
    ...
    for (idx_t j = 0; j < n; ++j) {
        ...
        for (idx_t i = 0; i < n; ++i) {
            ...
            Tmatrix(i, j) = static_cast<float>(0xFEE1DEAD);
        }
    ...
    // auto elapsedQR =
    //     std::chrono::duration_cast<std::chrono::nanoseconds>(endQR -
    //     startQR);
    ...
    //     // Compute ||Q'Q - I||_F
    //     norm_orth_1 =
    //         tlapack::lansy(tlapack::FROB_NORM, tlapack::UPPER_TRIANGLE,
    //         work);
